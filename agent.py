...
class Agent:
    ...
    def __init__(
        ...
        num_agents: int,
        gender: str = "male",
        job_name: str = "service_worker",
        automation_risk: float = 0.5,
        initial_cash_balance: float = 0.0,
        initial_structure_credits: float = 0.0,
        memory_limit: int = 20,
        memory_size: int = 5,
        ...
        self.automation_risk = automation_risk
        ...
        self.employed = True
        self.cash_balance = float(initial_cash_balance)
        self.structure_credit_balance = float(initial_structure_credits)
        self.last_job_loss_step: Optional[int] = None
        self.total_job_losses = 0
        self.unmet_needs = 0.0
        self.recent_wage_income = 0.0
        self.recent_structure_credit_income = 0.0
        self.recent_living_cost = 0.0
        self.recent_structure_credit_spent = 0.0
        self.recent_shortfall = 0.0
    ...
    def _build_nearby_agents_context(self, nearby_agents: List['Agent'], include_position: bool = True) -> str:
        ...
        for agent in nearby_agents:
            ...
            job_status = "employed" if agent.employed else "unemployed"
            role_text = f"works/worked as {agent.job_name} and is {job_status}"
            
            if include_position:
                nearby_info.append(
                    f"Agent {agent.id} ({agent.gender}) is at ({agent.position[0]}, {agent.position[1]}) "
                    f"and is {status}; {role_text}"
                )
            else:
                # Exclude position information for message prompts
                nearby_info.append(
                    f"Agent {agent.id} ({agent.gender}) is {status}; {role_text}"
                )
        return "\n".join(nearby_info)
    ...
    def start_new_step_economy(self):
        """Reset per-step economy flow values before the next simulation step."""
        self.recent_wage_income = 0.0
        self.recent_structure_credit_income = 0.0
        self.recent_living_cost = 0.0
        self.recent_structure_credit_spent = 0.0
    ...
    def lose_job(self, step: int):
        """Mark the agent as unemployed due to LLM automation."""
        if self.employed:
            self.employed = False
            self.last_job_loss_step = step
            self.total_job_losses += 1

    def receive_wage(self, amount: float):
        """Receive cash income from a remaining job."""
        if amount <= 0:
            return
        self.cash_balance += amount
        self.recent_wage_income += amount

    def receive_structure_credits(self, amount: float):
        """Receive structure-sustain currency for stabilizing actions."""
        if amount <= 0:
            return
        self.structure_credit_balance += amount
        self.recent_structure_credit_income += amount

    def pay_living_cost(self, amount: float):
        """Pay living cost from cash first, then from structure credits."""
        if amount <= 0:
            return

        self.recent_living_cost += amount

        paid_from_cash = min(self.cash_balance, amount)
        self.cash_balance -= paid_from_cash
        remaining = amount - paid_from_cash

        paid_from_structure = min(self.structure_credit_balance, remaining)
        self.structure_credit_balance -= paid_from_structure
        self.recent_structure_credit_spent += paid_from_structure
        remaining -= paid_from_structure

        self.recent_shortfall += remaining
        self.unmet_needs += remaining

    def _build_economy_section(self, economy_context: Optional[Dict]) -> str:
        """Build economy section for prompt if the economy layer is enabled."""
        if not economy_context or not economy_context.get('enabled'):
            return ""

        structure_credit_name = economy_context.get('structure_credit_name', 'structure credits')
        recent_job_losses = economy_context.get('recent_job_losses', [])
        if recent_job_losses:
            loss_lines = [
                f"Agent {entry['agent_id']} lost {entry['job_name']}"
                for entry in recent_job_losses[:5]
            ]
            recent_losses_text = "\n".join([f"  - {line}" for line in loss_lines])
        else:
            recent_losses_text = "  - No new LLM-driven job losses this step."

        return (
            "\n=== EMPLOYMENT AND STRUCTURE-SUSTAIN CURRENCY ===\n"
            f"Your job: {self.job_name}\n"
            f"Employment status: {'employed' if self.employed else 'unemployed'}\n"
            f"Automation risk for your job: {self.automation_risk:.2f}\n"
            f"Cash balance: {self.cash_balance:.2f}\n"
            f"{structure_credit_name} balance: {self.structure_credit_balance:.2f}\n"
            f"Accumulated basic-needs shortfall: {self.unmet_needs:.2f}\n"
            f"Recent wage income: {self.recent_wage_income:.2f}\n"
            f"Recent {structure_credit_name} income: {self.recent_structure_credit_income:.2f}\n"
            f"Recent living cost: {self.recent_living_cost:.2f}\n"
            f"Recent shortfall: {self.recent_shortfall:.2f}\n"
            f"Employed agents in society: {economy_context.get('employed_agents', 0)}/{self.num_agents}\n"
            f"Unemployment rate: {economy_context.get('unemployment_rate', 0.0):.2f}\n"
            "Recent LLM job losses:\n"
            f"{recent_losses_text}\n"
            f"In this world, agents can earn {structure_credit_name} by reducing disorder, "
            "helping others avoid fire, and keeping options open."
        )
    ...
    def create_message_prompt(
        ...
        economy_context: Optional[Dict] = None
    ) -> str:
        ...
        economy_section = self._build_economy_section(economy_context)
