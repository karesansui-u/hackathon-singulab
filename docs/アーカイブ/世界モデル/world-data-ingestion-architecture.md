...
- `fact_policy_event`
- `fact_political_profile`
- `fact_political_event`
...
## 9.1 国家の政治プロファイルを別レイヤで持つ

国家の振る舞いを現実寄りにするには、
`country = 1人格`
ではなく、
`country + political_profile + committee_roles`
で持つほうがよい。

そのため、baseline 側に少なくとも次の指標群を置く。
...
ここで `update_cost` は、
国家が危機に応じて前提を更新し、政策を組み替える際の
`前提のアップデートコスト`
である。

このプロファイルは `fact_political_profile` として、年次またはイベント時点で持つ。

## 9.2 政治委員会を入れるなら role table を分ける

国家ごとに 3 人または 5 人の政治委員会を導入する場合は、
役職を別テーブルで持つ。
...
`dim_political_role` に role の定義を置き、
各国家には
