...
このシミュレーションは、完全自由なLLM物語ではない。
同時に、エージェントの感情や行動を命令で決めるものでもない。

狙うのは中間である。

```text
固定するもの:
  出力形式、観測項目、分類語彙、時系列、シナリオ条件、制度設計の有無

固定しないもの:
  その条件を見た本人が何を知覚するか、どう感じるか、どう動くか、誰が反発するか
```

つまり、「こういう世界を見たい」という設計意図は、世界条件や制度条件として置いてよい。
ただし、「だから希望になる」「だから制度を支持する」「だから子どもを持つ」といった結果は命令しない。

この境界によって、再現性と創発性を両立する。
