...
def validate_domain_pack(resolved: ResolvedDomainPack) -> ValidationReport:
    ...
    agent_id_aliases = alias_values(config, "agent_id", ["agent_id", "エージェントID"])
    population_aliases = alias_values(config, "population_weight", ["population_weight", "人口重み_パーセント"])
    ...
    event_id_aliases = alias_values(config, "event_id", ["event_id", "イベントID"])
    start_step_aliases = alias_values(config, "start_step", ["start_step", "開始ステップ"])
